...
std::ostream& operator<<(std::ostream& os, const Position& pos) {
    ...
    for (Rank r = RANK_8; r >= RANK_1; --r) {
        for (File f = FILE_A; f <= FILE_H; ++f) os << " | " << PieceToChar[pos.piece_on(make_square(f, r))];
    ...
    os << "   a   b   c   d   e   f   g   h\n"
       ...
       << pos.key() << std::setfill(' ') << std::dec << "\nCheckers: ";

    for (Bitboard b = pos.checkers(); b;) os << UCI::square(pop_lsb(b)) << " ";
...
void Position::init() {
    ...
    for (File f = FILE_A; f <= FILE_H; ++f) Zobrist::enpassant[f] = rng.rand<Key>();

    for (int cr = NO_CASTLING; cr <= ANY_CASTLING; ++cr) Zobrist::castling[cr] = rng.rand<Key>();
...
void Position::set_state() const {
    ...
    for (Piece pc : Pieces)
        for (int cnt = 0; cnt < pieceCount[pc]; ++cnt) st->materialKey ^= Zobrist::psq[pc][cnt];
}
...
string Position::fen() const {
    ...
    for (Rank r = RANK_8; r >= RANK_1; --r) {
        for (File f = FILE_A; f <= FILE_H; ++f) {
            for (emptyCnt = 0; f <= FILE_H && empty(make_square(f, r)); ++f) ++emptyCnt;
...
void Position::update_slider_blockers(Color c) const {
    ...
    Bitboard snipers =
      ((attacks_bb<ROOK>(ksq) & pieces(QUEEN, ROOK)) | (attacks_bb<BISHOP>(ksq) & pieces(QUEEN, BISHOP))) & pieces(~c);
...
Bitboard Position::attackers_to(Square s, Bitboard occupied) const {
    ...
    return (pawn_attacks_bb(BLACK, s) & pieces(WHITE, PAWN)) | (pawn_attacks_bb(WHITE, s) & pieces(BLACK, PAWN))
         | (attacks_bb<KNIGHT>(s) & pieces(KNIGHT)) | (attacks_bb<ROOK>(s, occupied) & pieces(ROOK, QUEEN))
         | (attacks_bb<BISHOP>(s, occupied) & pieces(BISHOP, QUEEN)) | (attacks_bb<KING>(s) & pieces(KING));
}
...
bool Position::pseudo_legal(const Move m) const {
    ...
    if (type_of(m) != NORMAL)
        return checkers() ? MoveList<EVASIONS>(*this).contains(m) : MoveList<NON_EVASIONS>(*this).contains(m);
...
bool Position::gives_check(Move m) const {
    ...
    case PROMOTION : return attacks_bb(promotion_type(m), to, pieces() ^ from) & square<KING>(~sideToMove);
...
bool Position::pos_is_ok() const {
    ...
    if ((pieces(WHITE) & pieces(BLACK)) || (pieces(WHITE) | pieces(BLACK)) != pieces() || popcount(pieces(WHITE)) > 16
    ...
    for (Color c : {WHITE, BLACK})
        for (CastlingRights cr : {c & KING_SIDE, c & QUEEN_SIDE}) {
            ...
            if (piece_on(castlingRookSquare[cr]) != make_piece(c, ROOK)
                || castlingRightsMask[castlingRookSquare[cr]] != cr || (castlingRightsMask[square<KING>(c)] & cr) != cr)
