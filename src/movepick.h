...
inline int pawn_structure(const Position& pos) {
    // Use 4 of the 5 lowest bits for file and rank of both kings (each only the middest bit). Bit 3 is kept for pawn key.
    return (pos.pawn_key() & (PAWN_HISTORY_SIZE - 28)) ^ (int(pos.square<KING>(WHITE)) & 0x12)
         ^ ((int(pos.square<KING>(BLACK)) & 0x12) >> 1);
    ...
    //return (pos.pawn_key() & (PAWN_HISTORY_SIZE - 4)) ^ (pos.count<PAWN>(WHITE) & 1)
    //     ^ ((pos.count<PAWN>(BLACK) & 1) << 1);
