...
std::vector<std::string> setup_bench(const std::string& currentFen, std::istream& is, std::vector<int>& X) {

    std::vector<std::string> fens, list;
    std::string              go, token;

    // Assign default values to missing arguments
    std::string X1       = (is >> token) ? token : "0";
    std::string X2       = (is >> token) ? token : "0";
    std::string X3       = (is >> token) ? token : "0";
    ...
    X.push_back(std::stoi(X1));
    X.push_back(std::stoi(X2));
    X.push_back(std::stoi(X3));
