...
void ThreadPool::set(size_t requested) {
    ...
    if (threads.size() > 0)  // destroy any existing thread(s)
    {
        ...
        while (threads.size() > 0) delete threads.back(), threads.pop_back();
    }
    ...
    if (requested > 0)  // create new thread(s)
    {
        ...
        while (threads.size() < requested) threads.push_back(new Thread(threads.size()));
        clear();
...
Thread* ThreadPool::get_best_thread() const {
    ...
    for (Thread* th : threads) minScore = std::min(minScore, th->rootMoves[0].score);
    ...
    for (Thread* th : threads) votes[th->rootMoves[0].pv[0]] += thread_value(th);
