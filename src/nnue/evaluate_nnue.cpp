...
static void initialize() {
    ...
    for (std::size_t i = 0; i < LayerStacks; ++i) Detail::initialize(network[i]);
}
...
void hint_common_parent_position(const Position& pos) { featureTransformer->hint_common_access(pos); }
...
std::string trace(Position& pos) {
    ...
    ss << " NNUE network contributions " << (pos.side_to_move() == WHITE ? "(White to move)" : "(Black to move)")
       << std::endl
       << "+------------+------------+------------+------------+\n"
