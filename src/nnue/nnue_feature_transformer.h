...
class FeatureTransformer {
   ...
   public:
    ...
    static constexpr std::uint32_t get_hash_value() { return FeatureSet::HashValue ^ (OutputDimensions * 2); }
   ...
   private:
    ...
    template<Color Perspective, size_t N> void
    update_accumulator_incremental(const Position& pos, StateInfo* computed_st, StateInfo* states_to_update[N]) const {
        static_assert(N > 0);
...

        {
            ...
            while (states_to_update[i] == nullptr) --i;
            ...
            for (; i >= 0; --i) {
                ...
                for (; st2 != end_state; st2 = st2->previous)
                    FeatureSet::append_changed_indices<Perspective>(ksq, st2->dirtyPiece, removed[i], added[i]);
            }
        }
        ...
        for (IndexType j = 0; j < HalfDimensions / TileHeight; ++j) {
            ...
            for (IndexType k = 0; k < NumRegs; ++k) acc[k] = vec_load(&accTile[k]);
            ...
            for (IndexType i = 0; states_to_update[i]; ++i) {
                ...
                for (const auto index : removed[i]) {
                    ...
                    for (IndexType k = 0; k < NumRegs; ++k) acc[k] = vec_sub_16(acc[k], column[k]);
                }
                ...
                for (const auto index : added[i]) {
                    const IndexType offset = HalfDimensions * index + j * TileHeight;
                    auto            column = reinterpret_cast<const vec_t*>(&weights[offset]);
                    for (IndexType k = 0; k < NumRegs; ++k) acc[k] = vec_add_16(acc[k], column[k]);
                }

                // Store accumulator
                accTile =
                  reinterpret_cast<vec_t*>(&states_to_update[i]->accumulator.accumulation[Perspective][j * TileHeight]);
                for (IndexType k = 0; k < NumRegs; ++k) vec_store(&accTile[k], acc[k]);
            }
        }
        ...
        for (IndexType j = 0; j < PSQTBuckets / PsqtTileHeight; ++j) {
            ...
            for (std::size_t k = 0; k < NumPsqtRegs; ++k) psqt[k] = vec_load_psqt(&accTilePsqt[k]);
            ...
            for (IndexType i = 0; states_to_update[i]; ++i) {
                ...
                for (const auto index : removed[i]) {
                    ...
                    for (std::size_t k = 0; k < NumPsqtRegs; ++k) psqt[k] = vec_sub_psqt_32(psqt[k], columnPsqt[k]);
                }
                ...
                for (const auto index : added[i]) {
                    const IndexType offset     = PSQTBuckets * index + j * PsqtTileHeight;
                    auto            columnPsqt = reinterpret_cast<const psqt_vec_t*>(&psqtWeights[offset]);
                    for (std::size_t k = 0; k < NumPsqtRegs; ++k) psqt[k] = vec_add_psqt_32(psqt[k], columnPsqt[k]);
                }

                // Store accumulator
                accTilePsqt = reinterpret_cast<psqt_vec_t*>(
                  &states_to_update[i]->accumulator.psqtAccumulation[Perspective][j * PsqtTileHeight]);
                for (std::size_t k = 0; k < NumPsqtRegs; ++k) vec_store_psqt(&accTilePsqt[k], psqt[k]);
            }
        }
        ...
        for (IndexType i = 0; states_to_update[i]; ++i) {
            std::memcpy(states_to_update[i]->accumulator.accumulation[Perspective],
                        st->accumulator.accumulation[Perspective], HalfDimensions * sizeof(BiasType));
    ...
    template<Color Perspective> void update_accumulator_refresh(const Position& pos) const {
#ifdef VECTOR
        // Gcc-10.2 unnecessarily spills AVX2 registers if this array
        // is defined in the VECTOR code below, once in each branch
        vec_t      acc[NumRegs];
        ...
        for (IndexType j = 0; j < HalfDimensions / TileHeight; ++j) {
            ...
            for (IndexType k = 0; k < NumRegs; ++k) acc[k] = biasesTile[k];

            for (const auto index : active) {
                const IndexType offset = HalfDimensions * index + j * TileHeight;
                auto            column = reinterpret_cast<const vec_t*>(&weights[offset]);

                for (unsigned k = 0; k < NumRegs; ++k) acc[k] = vec_add_16(acc[k], column[k]);
            }

            auto accTile = reinterpret_cast<vec_t*>(&accumulator.accumulation[Perspective][j * TileHeight]);
            for (unsigned k = 0; k < NumRegs; k++) vec_store(&accTile[k], acc[k]);
        }

        for (IndexType j = 0; j < PSQTBuckets / PsqtTileHeight; ++j) {
            for (std::size_t k = 0; k < NumPsqtRegs; ++k) psqt[k] = vec_zero_psqt();
        ...
        for (std::size_t k = 0; k < PSQTBuckets; ++k) accumulator.psqtAccumulation[Perspective][k] = 0;
    ...
    template<Color Perspective> void hint_common_access_for_perspective(const Position& pos) const {
    ...
    template<Color Perspective> void update_accumulator(const Position& pos) const {
        ...
        if (oldest_st->accumulator.computed[Perspective]) {
            ...
            StateInfo* states_to_update[3] = {next, next == pos.state() ? nullptr : pos.state(), nullptr};
