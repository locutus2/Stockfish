...
namespace {
  ...
  Depth reduction(int i, Depth d, int mn, Value delta, Value rootDelta) {
    ...
    return (r + 1642 - int(delta) * 1024 / int(rootDelta)) / 1024 + (r - i > 1731);
  }
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          int lmrDepth = std::max(newDepth - reduction(improvement, depth, moveCount, delta, thisThread->rootDelta), 0);
      ...
      {
          Depth r = reduction(improvement, depth, moveCount, delta, thisThread->rootDelta);
