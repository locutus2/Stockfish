...
void Search::Worker::clear() {
    mainHistory[0].fill(66);
    mainHistory[1].fill(66);
...
moves_loop:  // When in check, search starts here
    ...
    MovePicker mp(pos, ttData.move, depth, &thisThread->mainHistory[ss->staticEval > 0],
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            else
            {
                ...
                history +=
                  68 * thisThread->mainHistory[ss->staticEval > 0][us][move.from_to()] / 32;
        ...
        if (capture)
            ...
            ss->statScore = thisThread->mainHistory[ss->staticEval > 0][us][move.from_to()]
                          + (*contHist[0])[movedPiece][move.to_sq()] - 2771;
        else
            ss->statScore = 2 * thisThread->mainHistory[ss->staticEval > 0][us][move.from_to()]
                          + (*contHist[0])[movedPiece][move.to_sq()]
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    MovePicker mp(pos, ttData.move, DEPTH_QS, &thisThread->mainHistory[ss->staticEval > 0],
                  &thisThread->lowPlyHistory, &thisThread->captureHistory, contHist,
                  &thisThread->pawnHistory, ss->ply);
