...
moves_loop:  // When in check, search starts here
    ...
    else if (priorCapture && prevSq != SQ_NONE)
    {
        ...
        int capBonus = 1080 + std::max(PieceValue[capturedPiece] - PieceValue[pos.piece_on(prevSq)], 0) / 8;
...
void update_all_stats(const Position&      pos,
    ...
    else
    {
        ...
        int capBonus = bonus + std::max(PieceValue[capturedPiece] - PieceValue[movedPiece], 0) / 8;
