...
//#define S(x,y,z) ((x)+1*((z)-(x)))
#define S(x,y,z) (z)

static constexpr std::array<int, CALLER_NB> R_log = { S(2834,2966,2894), S(2834,3015,2967), S(2834,2789,2894), S(2834,2551,2924), S(2834,2558,2699), S(2834,3027,2972), S(2834,2984,2870), S(2834,2868,2839), S(2834,2796,3078) };
static constexpr std::array<int, CALLER_NB> R_delta = { S(617,655,596), S(617,597,665), S(617,603,615), S(617,604,616), S(617,588,644), S(617,661,626), S(617,661,676), S(617,582,618), S(617,613,616) };
static constexpr std::array<int, CALLER_NB> R_imp = { S(194,191,199), S(194,203,202), S(194,205,184), S(194,208,176), S(194,201,194), S(194,192,201), S(194,190,183), S(194,188,191), S(194,197,193) };
static constexpr std::array<int, CALLER_NB> R_offset = { S(1027,998,1062), S(1027,1025,1084), S(1027,1044,1030), S(1027,1114,1031), S(1027,965,1004), S(1027,1070,1032), S(1027,960,998), S(1027,1004,1057), S(1027,1123,1036) };
           ...
           && !(limits.depth && mainThread && rootDepth >= limits.depth))
    {
        ...
        for (pvIdx = 0; pvIdx < multiPV; ++pvIdx)
        {
            ...
            while (true)
            {
                ...
                bestValue =
                  search<Root>(rootPos, ss, alpha, beta, adjustedDepth, false, ITERATIVE_DEEPENING);
