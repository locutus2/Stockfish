        ...
        if (!rootNode && pos.non_pawn_material(us) && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
        {
            ...
            else
            {
                int history = (*contHist[0])[movedPiece][to_sq(move)]
                            ...
                            + thisThread->pawnHistory[pawn_structure(pos)][us][from_to(move)];
...
void update_all_stats(const Position& pos,
    ...
    if (!pos.capture_stage(bestMove))
    {
        ...
        thisThread->pawnHistory[pawn_structure(pos)][us][from_to(bestMove)] << quietMoveBonus;

        // Decrease stats for all non-best quiet moves
        for (int i = 0; i < quietCount; ++i)
        {
            thisThread->pawnHistory[pawn_structure(pos)][us][from_to(quietsSearched[i])]
              << -bestMoveBonus;
