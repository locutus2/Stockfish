...
int Wbonus[3], Wmalus[3], Weval[3];

TUNE(SetRange(-SCALE, SCALE), Wbonus, Wmalus, Weval);
...
int stat_bonus(Depth d, bool PvNode, bool cutNode) {
    return std::min((291 * d - 350) * (SCALE + Wbonus[PvNode + 2 * cutNode]) / SCALE, 1200);
}

// History and stats update malus, based on depth
int stat_malus(Depth d, bool PvNode, bool cutNode) {
    return std::min((361 * d - 361) * (SCALE + Wmalus[PvNode + 2 * cutNode]) / SCALE, 1182);
}
...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        int bonus = std::clamp(-14 * int((ss - 1)->staticEval + ss->staticEval)
                                 * (SCALE + Weval[PvNode + 2 * cutNode]) / SCALE,
                               -1449, 1449);
