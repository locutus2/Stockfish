...
Value value_draw(const Thread* thisThread) { return VALUE_DRAW - 1 + Value(thisThread->nodes & 0x2); }
...
void Search::init() {
    ...
    for (int i = 1; i < MAX_MOVES; ++i) Reductions[i] = int((20.57 + std::log(Threads.size()) / 2) * std::log(i));
}
...
void Thread::search() {
    ...
    if (mainThread) {
        if (mainThread->bestPreviousScore == VALUE_INFINITE)
            ...
            for (int i = 0; i < 4; ++i) mainThread->iterValue[i] = mainThread->bestPreviousScore;
    }
...
template<NodeType nodeType> Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!ss->ttPv && depth < 9
        ...
        && eval >= beta && eval < 24923)  // larger than VALUE_KNOWN_WIN, but smaller than TB wins
        return eval;

    // Step 9. Null move search with verification search (~35 Elo)
    if (!PvNode && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 17329 && eval >= beta
        && eval >= ss->staticEval && ss->staticEval >= beta - 21 * depth + 258 && !excludedMove
...
moves_loop:  // When in check, search starts here
    ...
    if (ss->inCheck && !PvNode && ttCapture && (tte->bound() & BOUND_LOWER) && tte->depth() >= depth - 4
        && ttValue >= probCutBeta && abs(ttValue) <= VALUE_KNOWN_WIN && abs(beta) <= VALUE_KNOWN_WIN)
        return probCutBeta;
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE) {
        ...
        if (ss->ply < thisThread->rootDepth * 2) {
            ...
            if (!rootNode && depth >= 4 - (thisThread->completedDepth > 22) + 2 * (PvNode && tte->is_pv())
                && move == ttMove
        ...
        ss->statScore = 2 * thisThread->mainHistory[us][from_to(move)] + (*contHist[0])[movedPiece][to_sq(move)]
                      + (*contHist[1])[movedPiece][to_sq(move)] + (*contHist[3])[movedPiece][to_sq(move)] - 4006;
        ...
        if (depth >= 2 && moveCount > 1 + (PvNode && ss->ply <= 1)
            ...
            if (value > alpha && d < newDepth) {
                ...
                int bonus = value <= alpha ? -stat_bonus(newDepth) : value >= beta ? stat_bonus(newDepth) : 0;
