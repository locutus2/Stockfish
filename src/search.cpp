...
int correction_value(const Worker& w, const Position& pos, const Stack* const ss) {
    ...
    const int   pcv2   = shared.pawn_correction_entry<2>(pos)[us].pawn;
    const int   pcv3   = shared.pawn_correction_entry<3>(pos)[us].pawn;
    ...
    return 3835 * (pcv0 + pcv1 + pcv2 + pcv3) + 10569 * micv + 12906 * (wnpcv + bnpcv) + cntcv;
}
...
void update_correction_history(const Position& pos,
    ...
    shared.pawn_correction_entry<2>(pos)[us].pawn << bonus;
    shared.pawn_correction_entry<3>(pos)[us].pawn << bonus;
