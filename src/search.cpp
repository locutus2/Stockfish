...
void  update_continuation_histories(Stack* ss, Search::Worker& workerThread, Piece pc, Square from, Square to, int bonus);
        ...
        && (cutNode == (ttData.value >= beta) || depth > 8))
    {
        ...
        if (ttData.move && ttData.value >= beta)
        {
            ...
            if (prevSq != SQ_NONE && (ss - 1)->moveCount <= 2 && !priorCapture)
                update_continuation_histories(ss - 1, *this, pos.piece_on(prevSq), prevFromSq, prevSq,
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (depth >= 2 && moveCount > 1)
        {
            ...
            if (value > alpha && d < newDepth)
            {
                ...
                update_continuation_histories(ss, *this, movedPiece, move.from_sq(), move.to_sq(), bonus * 1427 / 1024);
            }
        }
...
void update_all_stats(const Position&      pos,
    ...
    if (prevSq != SQ_NONE && ((ss - 1)->moveCount == 1 + (ss - 1)->ttHit) && !pos.captured_piece())
        update_continuation_histories(ss - 1, workerThread, pos.piece_on(prevSq), prevFromSq, prevSq, -malus * 919 / 1024);
...
void update_continuation_histories(Stack* ss, Search::Worker& workerThread, Piece pc, Square from, Square to, int bonus) {
    ...
    if (bonus > 0)
        (*&workerThread.continuationHistory[0][0][pc][from])[pc][to] << bonus;
}
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    update_continuation_histories(ss, workerThread, pos.moved_piece(move), move.from_sq(), move.to_sq(), bonus * 853 / 1024);
