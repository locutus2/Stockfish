...
namespace {
  ...
  Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta, Value material) {
    int r = Reductions[d] * Reductions[mn] + 1038 - material / 16;
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          int lmrDepth = std::max(newDepth - reduction(improving, depth, moveCount, delta, thisThread->rootDelta, pos.non_pawn_material()), 0);
      ...
      Depth r = reduction(improving, depth, moveCount, delta, thisThread->rootDelta, pos.non_pawn_material());
