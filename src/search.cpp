...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    ttCapture = ss->ttMove && pos.capture_stage(ss->ttMove);
             ...
             && ttValue < probCutBeta))
    {
        ...
        MovePicker mp(pos, ss->ttMove, ss->inCheck || (ss-2)->inCheck ? MOVE_NONE : (ss-2)->ttMove, probCutBeta - ss->staticEval, &captureHistory);
...
moves_loop: // When in check, search starts here
    ...
    MovePicker mp(pos, ss->ttMove, ss->inCheck || (ss-2)->inCheck ? MOVE_NONE : (ss-2)->ttMove, depth, &thisThread->mainHistory,
                                      &captureHistory,
                                      contHist,
                                      countermove,
                                      ss->killers);
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    tte = TT.probe(posKey, ss->ttHit);
    ttValue = ss->ttHit ? value_from_tt(tte->value(), ss->ply, pos.rule50_count()) : VALUE_NONE;
    ss->ttMove = ss->ttHit ? tte->move() : MOVE_NONE;
