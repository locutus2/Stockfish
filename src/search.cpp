...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && (tte->bound() & (ttValue >= beta ? BOUND_LOWER : BOUND_UPPER)))
    {
        ...
        if (ttMove)
        {
            ...
            else if (!ttCapture)
            {
                ...
                thisThread->mainHistory[us][from_to(ttMove)] << 2 * penalty;
    ...
    if (is_ok((ss-1)->currentMove) && !(ss-1)->inCheck && !priorCapture)
    {
        ...
        thisThread->mainHistory[~us][from_to((ss-1)->currentMove)] << 2 * bonus;
    }
...
moves_loop: // When in check, search starts here
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (!pos.capture(bestMove))
    {
        ...
        {
            thisThread->mainHistory[us][from_to(quietsSearched[i])] << -2 * bonus2;
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus) {
    ...
    Color us = pos.side_to_move();
    Thread* thisThread = pos.this_thread();
    thisThread->mainHistory[us][from_to(move)] << 2 * bonus;
