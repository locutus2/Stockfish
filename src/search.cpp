...
namespace {
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus, bool PvNode, bool cutNode);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && (tte->bound() & (ttValue >= beta ? BOUND_LOWER : BOUND_UPPER)))
    {
        ...
        if (ttMove)
        {
            ...
            else if (!ttCapture)
            {
                ...
                thisThread->mainHistory[node_type(PvNode, cutNode)][us][from_to(ttMove)] << 2 * penalty;
    ...
    if (is_ok((ss-1)->currentMove) && !(ss-1)->inCheck && !priorCapture)
    {
        ...
        thisThread->mainHistory[node_type(PvNode, cutNode)][~us][from_to((ss-1)->currentMove)] << 2 * bonus;
    }
...
moves_loop: // When in check, search starts here
    ...
    MovePicker mp(pos, ttMove, depth, &thisThread->mainHistory[node_type(PvNode, cutNode)],
                                      &captureHistory,
                                      contHist,
                                      countermove,
                                      ss->killers);
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          else
          {
              ...
              history += 2 * thisThread->mainHistory[node_type(PvNode, cutNode)][us][from_to(move)];
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          ss->statScore =  2 * thisThread->mainHistory[node_type(PvNode, cutNode)][us][from_to(move)]
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    // will be generated.
    Square prevSq = to_sq((ss-1)->currentMove);
    MovePicker mp(pos, ttMove, depth, &thisThread->mainHistory[node_type(PvNode, false)],
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (!pos.capture(bestMove))
    {
        ...
        update_quiet_stats(pos, ss, bestMove, bonus2, PvNode, cutNode);

        // Decrease stats for all non-best quiet moves
        for (int i = 0; i < quietCount; ++i)
        {
            thisThread->mainHistory[node_type(PvNode, cutNode)][us][from_to(quietsSearched[i])] << -2 * bonus2;
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus, bool PvNode, bool cutNode) {
    ...
    Color us = pos.side_to_move();
    Thread* thisThread = pos.this_thread();
    thisThread->mainHistory[node_type(PvNode, cutNode)][us][from_to(move)] << 2 * bonus;
