...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    ButterflyHistory &mainHistory = thisThread->mainHistory[type_of(pos.piece_on(prevSq)) == KING];
    ...
    if (is_ok((ss-1)->currentMove) && !(ss-1)->inCheck && !priorCapture)
    {
        ...
        thisThread->mainHistory[type_of(pos.piece_on(to_sq((ss-2)->currentMove))) == KING][~us][from_to((ss-1)->currentMove)] << bonus;
    }
...
moves_loop: // When in check, search starts here
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    ButterflyHistory &mainHistory = thisThread->mainHistory[type_of(pos.piece_on(to_sq((ss-1)->currentMove))) == KING];
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (!pos.capture(bestMove))
    {
        ...
        {
            mainHistory[us][from_to(quietsSearched[i])] << -bonus2;
