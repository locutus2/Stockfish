...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (ss->ply < thisThread->rootDepth * 2)
        {
            ...
            {
                Value singularBeta  = ttData.value - (55 + 80 * (ss->ttPv && !PvNode)) * depth / 64;
    ...
    {
        int bonusScale = (116 * (depth > 5) + 40 * !allNode + 188 * ((ss - 1)->moveCount > 8)
        ...
        thisThread->mainHistory[~us][((ss - 1)->currentMove).from_to()] << scaledBonus * 209 / 1024;
