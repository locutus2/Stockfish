...
moves_loop:  // When in check, search starts here
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < 7 && !ss->inCheck)
                {
                    Value futilityValue = ss->staticEval + 242 + 230 * lmrDepth
                                        + PieceValue[capturedPiece] + 133 * captHist / 2048;
                    if (futilityValue <= alpha)
                        continue;
                }

                // SEE based pruning for captures and checks
                int seeHist = std::clamp(captHist / 64, -138 * depth, 135 * depth);
    ...
    else if (priorCapture && prevSq != SQ_NONE)
    {
        ...
        thisThread->captureHistory[pos.piece_on(prevSq)][prevSq][type_of(capturedPiece)] << 2200;
    }
...
void update_all_stats(const Position&      pos,
    ...
    else
    {
        ...
        captureHistory[moved_piece][bestMove.to_sq()][captured] << bonus * 1187 / 512;
    }
    ...
    for (Move move : capturesSearched)
    {
        ...
        captureHistory[moved_piece][move.to_sq()][captured] << -malus * 1377 / 512;
    }
}
