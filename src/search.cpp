...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (((ss - 1)->currentMove).is_ok() && !(ss - 1)->inCheck && !priorCapture)
    {
        ...
        thisThread->mainHistory[~us][((ss - 1)->currentMove).from_to()] << bonus * 141 / 1024;
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        if (!rootNode && pos.non_pawn_material(us) && !is_loss(bestValue))
        {
            ...
            else
            {
                ...
                history += 17 * thisThread->mainHistory[us][move.from_to()];
        ...
        if (capture)
            ...
            ss->statScore = 16 * thisThread->mainHistory[us][move.from_to()]
                          + (*contHist[0])[movedPiece][move.to_sq()]
    ...
    else if (!priorCapture && prevSq != SQ_NONE)
    {
        ...
        thisThread->mainHistory[~us][((ss - 1)->currentMove).from_to()] << scaledBonus * 27 / 32768;
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::Worker& workerThread, Move move, int bonus) {
    ...
    workerThread.mainHistory[us][move.from_to()]
      << bonus / 8;  // Untuned to prevent duplicate effort
