...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      bool C =  (    pcheck(PARAMS[0], ss->inCheck)
             ...
             && pcheck(PARAMS[46], depth <= 1)
             && pcheck(PARAMS[47], depth <= 2)
             && pcheck(PARAMS[48], depth <= 3)
             && pcheck(PARAMS[49], depth <= 4)
             && pcheck(PARAMS[50], depth <= 5)
             && pcheck(PARAMS[51], depth <= 6)
             && pcheck(PARAMS[52], depth <= 7)
             && pcheck(PARAMS[53], depth <= 8)
             && pcheck(PARAMS[54], depth <= 9)
             && pcheck(PARAMS[55], depth <= 10)
             && pcheck(PARAMS[56], depth <= 11)
             && pcheck(PARAMS[57], depth <= 12)
             && pcheck(PARAMS[58], depth <= 13)
             && pcheck(PARAMS[59], moveCount == 0)
             && pcheck(PARAMS[60], moveCount == 1)
             && pcheck(PARAMS[61], moveCount > 1)
             && pcheck(PARAMS[62], (ss-1)->moveCount == 0)
             && pcheck(PARAMS[63], (ss-1)->moveCount == 1)
             && pcheck(PARAMS[64], (ss-1)->moveCount > 1)
             && pcheck(PARAMS[65], (ss-2)->moveCount == 0)
             && pcheck(PARAMS[66], (ss-2)->moveCount == 1)
             && pcheck(PARAMS[67], (ss-2)->moveCount > 1)
             );
