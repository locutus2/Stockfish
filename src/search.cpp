...
namespace {
  ...
  Depth reduction(Depth d, int mn, Value delta, Value rootDelta) {
    int r = Reductions[d] * Reductions[mn];
    return (r + 1449 - int(delta) * 937 / int(rootDelta)) / 1024 + (r > 941);
  }
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      Depth r = reduction(depth, moveCount, delta, thisThread->rootDelta);
