  ...
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    else
    {
        ...
        ttWriter.write(posKey, VALUE_NONE, ss->ttPv, BOUND_NONE, DEPTH_UNSEARCHED, Move::none(),
                       (unadjustedStaticEval * 31 + ss->staticEval) / 32, tt.generation());
    }
        ...
        while ((move = mp.next_move()) != Move::none())
        {
            ...
            if (value >= probCutBeta)
            {
                ...
                ttWriter.write(posKey, value_to_tt(value, ss->ply), ss->ttPv, BOUND_LOWER,
                               probCutDepth + 1, move, (unadjustedStaticEval * 31 + ss->staticEval) / 32, tt.generation());
    ...
    if (!excludedMove && !(rootNode && thisThread->pvIdx))
        ttWriter.write(posKey, value_to_tt(bestValue, ss->ply), ss->ttPv,
                       ...
                       : PvNode && bestMove ? BOUND_EXACT
                                            : BOUND_UPPER,
                       depth, bestMove, (unadjustedStaticEval * 31 + ss->staticEval) / 32, tt.generation());
        ...
        if (bestValue >= beta)
        {
            ...
            if (!ss->ttHit)
                ttWriter.write(posKey, value_to_tt(bestValue, ss->ply), false, BOUND_LOWER,
                               DEPTH_UNSEARCHED, Move::none(), (unadjustedStaticEval * 31 + ss->staticEval) / 32,
                               tt.generation());
            return bestValue;
        }
