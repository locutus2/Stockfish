...
moves_loop: // When in check, search starts from here
    ...
    Move countermove = thisThread->counterMoves[pos.piece_on(prevSq)][prevSq]->move;

    if (pos.moved_piece(countermove) != thisThread->counterMoves[pos.piece_on(prevSq)][prevSq]->piece)
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus) {
    ...
    {
        Square prevSq = to_sq((ss-1)->currentMove);
        thisThread->counterMoves[pos.piece_on(prevSq)][prevSq] = PieceMove(pos.moved_piece(move), move);
    }
  }
