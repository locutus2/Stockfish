...
void Thread::search() {
  ...
  for(RootMove& rm : rootMoves)
      rm.countBestMove = rootPos.capture(rm.pv[0]) ? captureHistory[rootPos.moved_piece(rm.pv[0])][to_sq(rm.pv[0])][type_of(rootPos.piece_on(to_sq(rm.pv[0])))]
                                                   : mainHistory[us][from_to(rm.pv[0])];
