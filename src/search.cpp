...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool narrowSearch);
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          while (true)
          {
              ...
              bestValue = Stockfish::search<Root>(rootPos, ss, alpha, beta, adjustedDepth, false, false);
        ...
        while ((move = mp.next_move()) != MOVE_NONE)
            if (move != excludedMove && pos.legal(move))
            {
                ...
                if (value >= probCutBeta)
                    value = -search<NonPV>(pos, ss+1, -probCutBeta, -probCutBeta+1, depth - 4, !cutNode, false);
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          value = -search<NonPV>(pos, ss+1, -(alpha+1), -alpha, d, true, narrowSearch);

          // Do full depth search when reduced LMR search fails high
          if (value > alpha && d < newDepth)
          {
              const bool doDeeperSearch = value > (alpha + 64 + 11 * (newDepth - d));
              value = -search<NonPV>(pos, ss+1, -(alpha+1), -alpha, newDepth + doDeeperSearch, !cutNode, narrowSearch);
      ...
      {
              value = -search<NonPV>(pos, ss+1, -(alpha+1), -alpha, newDepth, !cutNode, narrowSearch);
      }
      ...
      if (PvNode && (moveCount == 1 || (value > alpha && (rootNode || value < beta))))
      {
          ...
          value = -search<PV>(pos, ss+1, -beta, -alpha,
                              std::min(maxNextDepth, newDepth), false, false);
      }
