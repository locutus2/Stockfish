...
void MovePicker::score() {
    ...
    for (auto& m : *this)
        ...
        else if constexpr (Type == QUIETS)
        {
            ...
            m.value += (3 * (*pawnHistory)[pawn_structure(pos)][pc][to]
                        + (*continuationHistory[1])[pc][to] + (*continuationHistory[5])[pc][to])
                     / 2;
            m.value += (3 * (*continuationHistory[0])[pc][to] + (*continuationHistory[2])[pc][to]
                        + 3 * (*continuationHistory[3])[pc][to])
                     / 4;
...
top:
    ...
    case QUIET_INIT :
        if (!skipQuiets)
        {
            ...
            score<QUIETS>();
            partial_insertion_sort(cur, endMoves, 5506 - 2979 * depth - 8 * depth * depth);
        }
