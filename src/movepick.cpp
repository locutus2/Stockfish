...
MovePicker::MovePicker(const Position& p, Move ttm, Depth d, const ButterflyHistory* mh,
           ...
           : pos(p), mainHistory(mh), captureHistory(cph), continuationHistory(ch),
             ttMove(ttm), refutations{{killers[0], 0}, {killers[1], 0}, {cm, 0}, {cm2, 0}}, depth(d)
{
  assert(d > 0);
...
top:
  ...
  case GOOD_CAPTURE:
      ...
      // If the second countermove is the same as a killer, skip it
      if (   refutations[0].move == refutations[3].move
          || refutations[1].move == refutations[3].move)
          --endMoves;

      // If the first countermove is the same as a killer, skip it
