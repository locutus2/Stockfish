...
MovePicker::MovePicker(const Position&              p,
                       ...
                       int                          pl,
                       const CapturePieceToHistory* cpch) :
    pos(p),
    mainHistory(mh),
    lowPlyHistory(lph),
    captureHistory(cph),
    continuationHistory(ch),
    captureContinuationHistory(cpch),
    pawnHistory(ph),
    ttMove(ttm),
    depth(d),
    ply(pl) {
...
MovePicker::MovePicker(const Position& p, Move ttm, int th, const CapturePieceToHistory* cph, const CapturePieceToHistory* cpch) :
    pos(p),
    captureHistory(cph),
    captureContinuationHistory(cpch),
    ttMove(ttm),
    ...
    for (auto& m : *this)
    {
        ...
        if constexpr (Type == CAPTURES)
            m.value = (*captureHistory)[pc][to][type_of(capturedPiece)]
                    + 2 * (*captureContinuationHistory)[pc][to][type_of(capturedPiece)]
