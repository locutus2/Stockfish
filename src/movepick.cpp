...
void MovePicker::score() {
    ...
    for (auto& m : *this)
        ...
        else if constexpr (Type == QUIETS)
        {
            ...
            // histories
            m.value = (9 * (*mainHistory)[pos.side_to_move()][from_to(m)]
                       + 7 * (*pawnHistory)[pawn_structure(pos)][pc][to]
                       + 3 * (*continuationHistory[0])[pc][to] + (*continuationHistory[3])[pc][to]
                       + (*continuationHistory[4])[pc][to])
                    / 4;
            m.value += ((*continuationHistory[1])[pc][to] + (*continuationHistory[2])[pc][to]
