...
constexpr Value WDL_to_value[] = {-VALUE_MATE + MAX_PLY + 1, VALUE_DRAW - 2, VALUE_DRAW, VALUE_DRAW + 2,
                                  VALUE_MATE - MAX_PLY - 1};

template<typename T, int Half = sizeof(T) / 2, int End = sizeof(T) - 1> inline void swap_endian(T& x) {
   ...
   public:
    ...
    uint8_t* map(void** baseAddress, uint64_t* mapping, TBType type) {
        ...
        *baseAddress = mmap(nullptr, statbuf.st_size, PROT_READ, MAP_SHARED, fd, 0);
        ...
        HANDLE fd = CreateFileA(fname.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
...
template<> TBTable<WDL>::TBTable(const std::string& code) :
    TBTable() {
...
template<> TBTable<DTZ>::TBTable(const TBTable<WDL>& wdl) :
    TBTable() {
...
void TBTables::add(const std::vector<PieceType>& pieces) {
    ...
    for (PieceType pt : pieces) code += PieceToChar[pt];
...
int map_score(TBTable<DTZ>* entry, File f, int value, WDLScore wdl) {
    ...
    if ((wdl == WDLWin && !(flags & TBFlag::WinPlies)) || (wdl == WDLLoss && !(flags & TBFlag::LossPlies))
...
Ret do_probe_table(const Position& pos, T* entry, WDLScore wdl, ProbeState* result) {
    ...
    if (entry->hasPawns) {
        ...
        do squares[size++] = pop_lsb(b) ^ flipSquares;
        while (b);
    ...
    if (file_of(squares[0]) > FILE_D)
        for (int i = 0; i < size; ++i) squares[i] = flip_file(squares[i]);
    ...
    if (entry->hasPawns) {
        ...
        for (int i = 1; i < leadPawnsCnt; ++i) idx += Binomial[i][MapPawns[squares[i]]];
    ...
    if (rank_of(squares[0]) > RANK_4)
        for (int i = 0; i < size; ++i) squares[i] = flip_rank(squares[i]);
    ...
    for (int i = 0; i < d->groupLen[0]; ++i) {
        ...
        if (off_A1H8(squares[i]) > 0)  // A1-H8 diagonal flip: SQ_A3 -> SQ_C1
            for (int j = i; j < size; ++j) squares[j] = Square(((squares[j] >> 3) | (squares[j] << 3)) & 63);
        break;
    }
    ...
    if (entry->hasUniquePieces) {
        ...
        // First two pieces are on a1-h8 diagonal, third below
            ...
            idx = 6 * 63 * 62 + 4 * 28 * 62 + rank_of(squares[0]) * 7 * 28 + (rank_of(squares[1]) - adjust1) * 28
                + MapB1H1H7[squares[2]];
...
uint8_t* set_sizes(PairsData* d, uint8_t* data) {
    ...
    for (int i = 0; i < base64_size; ++i) d->base64[i] <<= 64 - i - d->minSymLen;  // Right-padding to 64 bits
...
template<typename T> void set(T& e, uint8_t* data) {
    ...
    for (File f = FILE_A; f <= maxFile; ++f) {
        ...
        for (int i = 0; i < sides; i++) *e.get(i, f) = PairsData();

        int order[][2] = {{*data & 0xF, pp ? *(data + 1) & 0xF : 0xF}, {*data >> 4, pp ? *(data + 1) >> 4 : 0xF}};
        data += 1 + pp;

        for (int k = 0; k < e.pieceCount; ++k, ++data)
            for (int i = 0; i < sides; i++) e.get(i, f)->pieces[k] = Piece(i ? *data >> 4 : *data & 0xF);

        for (int i = 0; i < sides; ++i) set_groups(e, e.get(i, f), order[i], f);
    }

    data += (uintptr_t) data & 1;  // Word alignment

    for (File f = FILE_A; f <= maxFile; ++f)
        for (int i = 0; i < sides; i++) data = set_sizes(e.get(i, f), data);
...
template<bool CheckZeroingMoves> WDLScore search(Position& pos, ProbeState* result) {
...
void Tablebases::init(const std::string& paths) {
    ...
    for (auto s : diagonal) MapA1D1D4[s] = code++;
    ...
    for (auto p : bothOnDiagonal) MapKK[p.first][p.second] = code++;
    ...
    for (PieceType p1 = PAWN; p1 < KING; ++p1) {
        ...
        for (PieceType p2 = PAWN; p2 <= p1; ++p2) {
            ...
            for (PieceType p3 = PAWN; p3 < KING; ++p3) TBTables.add({KING, p1, p2, KING, p3});
            ...
            for (PieceType p3 = PAWN; p3 <= p2; ++p3) {
                ...
                for (PieceType p4 = PAWN; p4 <= p3; ++p4) {
                    ...
                    for (PieceType p5 = PAWN; p5 <= p4; ++p5) TBTables.add({KING, p1, p2, p3, p4, p5, KING});

                    for (PieceType p5 = PAWN; p5 < KING; ++p5) TBTables.add({KING, p1, p2, p3, p4, KING, p5});
                }

                for (PieceType p4 = PAWN; p4 < KING; ++p4) {
                    TBTables.add({KING, p1, p2, p3, KING, p4});

                    for (PieceType p5 = PAWN; p5 <= p4; ++p5) TBTables.add({KING, p1, p2, p3, KING, p4, p5});
                }
            }

            for (PieceType p3 = PAWN; p3 <= p1; ++p3)
                for (PieceType p4 = PAWN; p4 <= (p1 == p3 ? p2 : p3); ++p4) TBTables.add({KING, p1, p2, KING, p3, p4});
        }
    }
